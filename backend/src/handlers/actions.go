...
func (srv *Server) HandleImportUsers(w http.ResponseWriter, r *http.Request) {
	...
	logFields["provider id"] = service.ProviderPlatformID
	...
	for _, program := range programs {
		...
		for _, userMapping := range userMappings {
			...
			for _, milestone := range milestones {
				logFields["milestone id"] = milestone.ExternalID
				ms := models.Milestone{
...
func (srv *Server) getProgramsAndMappingsForProvider(providerID uint) ([]models.Program, []models.ProviderUserMapping, error) {
	...
	if err != nil {
		log.WithField("provider id", providerID).Errorf("Error getting user mappings for provider: %v", err)
		return nil, nil, err
	}
	programs, err := srv.Db.GetProgramByProviderPlatformID(int(providerID))
	if err != nil {
		log.WithField("provider id", providerID).Errorf("Error getting programs for provider: %v", err)
		return nil, nil, err
	}
...
func (srv *Server) HandleImportActivity(w http.ResponseWriter, r *http.Request) {
	...
	for _, program := range programs {
		...
		for _, act := range activity {
			logFields["user id"] = userMap[act.ExternalUserID]
